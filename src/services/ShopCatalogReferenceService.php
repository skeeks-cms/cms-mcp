...
class ShopCatalogReferenceService extends AbstractCmsService
{
    ...
    protected function mutateBrand(ShopBrand $m,array $a): array { $this->apply($m,$a,$this->brandWritable);return $this->brandData($this->save($m,'Brand validation failed'),true); }
    protected function mutateCollection(ShopCollection $m,array $a): array { $this->apply($m,$a,$this->collectionWritable);if(isset($a['sticker_ids'])){$m->shopCollectionStickers=$a['sticker_ids'];}return $this->collectionData($this->save($m,'Collection validation failed'),true); }
