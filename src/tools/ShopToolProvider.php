...
class ShopToolProvider extends Component implements McpToolProviderInterface
{
    ...
    protected function productUpsertSchema(): array{return $this->object(['match'=>$this->productResolveSchema(),'content_id'=>['type'=>'integer'],'element'=>['type'=>'object'],'product'=>['type'=>'object'],'properties'=>['type'=>'object'],'publish'=>['type'=>'boolean'],'image_ids'=>['type'=>'array','items'=>['type'=>'integer'],'description'=>'Ordered images: the first id becomes element.image_id, remaining unique ids form the gallery. An empty array clears only the gallery unless element.image_id is explicitly null.'],'file_ids'=>['type'=>'array','items'=>['type'=>'integer']],'allow_create'=>['type'=>'boolean'],'allow_update'=>['type'=>'boolean']],['match']);}
