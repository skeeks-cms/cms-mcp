...
class CrmToolProvider extends Component implements McpToolProviderInterface
{
    ...
    public function getTools(): array
    {
        ...
        $tools = array_merge($tools, $this->crud('cms_task', 'задачи', 'cms.task', $task, 'task', 'taskStats', ['list' => $this->taskListSchema(), 'stats' => $this->taskStatsSchema(), 'create' => $this->taskMutationSchema(), 'update' => $this->taskMutationSchema(['id'])]));
        $tools[] = $this->tool('cms_task_action', 'Действие как в интерфейсе: accept — принять; start — начать/продолжить и автоматически включить рабочее время; pause — остановить задачу; complete — завершить свою работу (готова или на проверку); approve — подтвердить результат автором; reopen — вернуть на доработку; cancel — отменить. Проверки прав, статуса и интервалы выполняются атомарно. Пауза/завершение задачи не выключает рабочий день. Повтор действия не создаёт интервал повторно.', 'cms.task.write', [$task, 'taskAction'], $this->object(['id' => ['type' => 'integer'], 'action' => ['type' => 'string', 'enum' => ['accept', 'start', 'pause', 'complete', 'approve', 'reopen', 'cancel']]], ['id', 'action']));
        $tools[] = $this->tool('cms_task_status_repair', 'Только администратор: восстановить неизвестный статус по последнему корректному значению журнала. Только принята/на паузе и без открытых интервалов; корректные статусы не меняет.', 'cms.task.write', [$task, 'taskRepairStatus'], $this->idSchema());
    ...
    protected function crud(string $prefix, string $label, string $scope, $service, string $methodPrefix, string $statsMethod = null, array $schemas = []): array
    {
        ...
        if ($prefix === 'cms_task') { $createDescription .= ' Статус определяется сервером; для начала работы используйте cms_task_action. Перед созданием разрешите компанию, проект и исполнителя через соответствующие list-инструменты; неоднозначный выбор уточните у пользователя.'; }
        $updateDescription = 'Редактирование '.$label.'. Автор системных полей не принимается из MCP.';
        if ($prefix === 'cms_task') { $updateDescription .= ' Поле status недоступно для записи; используйте cms_task_action.'; }
    ...
    protected function taskMutationSchema(array $required = []): array
    {
        $schema = $this->mutation($required);
        unset($schema['properties']['status']);
        $schema['properties']['attributes']['description'] = 'Поля задачи, кроме status. Статус меняется только через cms_task_action.';
        return $schema;
    }
